...
- **M4 (Agent Security & Defense-in-Depth):**
  ...
  - **Typed Tool Allow-List (`ToolRegistry`):** strict schema validation and role-based access control (`PUBLIC` / `MANAGER` / `SYSTEM`) preventing arbitrary or unauthorized tool execution. The live workflow includes these active tools:
    ...
    - `find_recent_incidents` (`SYSTEM`) is exposed to the Classification Agent as a genuinely agentic tool call, not a deterministic pre-fetch — the model itself decides whether a report looks like a recurring pattern before calling it. Matching is by **semantic similarity of location** (pgvector cosine similarity over an embedded location string, not exact-text matching — e.g. "Level 4 Room 402" and "Level 4 Room 404" can both match). Results, including each match's assigned team and any operational note, feed into the Classification, Priority, and Assignment agents as read-only reference context — usable to disambiguate an unclear category or justify escalating a recurring/unresolved pattern, never to downgrade a hazard. The `recent_incident_lookup` trace node is always recorded, whether or not the model calls the tool, so "the model judged this a one-off" is never indistinguishable from "the tool silently never ran."
...
- A second incident report at a semantically similar location to a recent report (e.g. "Level 4 Room 404" vs. a prior "Level 4 Room 402" leak — not an exact text match) is recognized by the Classification Agent, which calls `find_recent_incidents` itself and enriches Classification, Priority, and Assignment with that history before a decision is made.
